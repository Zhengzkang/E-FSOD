...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels, ious):
        ...
        label_mask = torch.eq(labels, labels.T).float().cuda() #y是否等于ｙT，返回True/False－>1/0
        # div将数组input与数组other对应元素做除法,matmul矩阵乘法
        similarity = torch.div(
            torch.matmul(features, features.T), self.temperature) #(z*zT)/t
